 ...
 * ```
 */
const Viewer360 = defineComponent({
    name: 'Viewer360',
    props: {
        images: { type: Array, required: true },
        highResImages: { type: Array, default: undefined },
        ...
        rotationDirection: { type: String, default: VIEWER_360_DEFAULTS.rotationDirection },
        initialResolution: { type: Number, default: undefined },
        dprCap: { type: Number, default: VIEWER_360_DEFAULTS.dprCap },
        zoomLevels: { type: Array, default: () => [...VIEWER_360_DEFAULTS.zoomLevels] },
    ...
    emits: {
        ...
        ready: (_payload) => true,
        frameChange: (_detail) => true,
        zoomChange: (_detail) => true,
        autoRotateChange: (_detail) => true,
        close: (_payload) => true,
        error: (_detail) => true,
    ...
    setup(props, { emit, expose }) {
        ...
        const syncArrayProps = (el) => {
            el.images = props.images;
            if (props.highResImages)
                el.highResImages = props.highResImages;
            el.zoomLevels = props.zoomLevels;
        };
        watch(() => props.images, () => { if (elRef.value)
            elRef.value.images = props.images; }, { deep: true });
        watch(() => props.highResImages, () => { if (elRef.value && props.highResImages)
            elRef.value.highResImages = props.highResImages; }, { deep: true });
        watch(() => props.zoomLevels, () => { if (elRef.value)
            elRef.value.zoomLevels = props.zoomLevels; }, { deep: true });
        // ── Lifecycle ─────────────────────────────────────────────────────────────
        onMounted(() => {
            ...
            on('viewer360:framechange', (e) => emit('frameChange', e.detail));
            on('viewer360:zoomchange', (e) => emit('zoomChange', e.detail));
            on('viewer360:autorotatechange', (e) => emit('autoRotateChange', e.detail));
            on('viewer360:close', () => emit('close'));
            on('viewer360:error', (e) => emit('error', e.detail));
        });
        onUnmounted(() => {
            const el = elRef.value;
            if (el)
        ...
        expose({
            showFrame: (f) => { var _a; return (_a = elRef.value) === null || _a === void 0 ? void 0 : _a.showFrame(f); },
            setZoom: (level) => { var _a; return (_a = elRef.value) === null || _a === void 0 ? void 0 : _a.setZoom(level); },
            startAutoRotate: () => { var _a; return (_a = elRef.value) === null || _a === void 0 ? void 0 : _a.startAutoRotate(); },
            stopAutoRotate: () => { var _a; return (_a = elRef.value) === null || _a === void 0 ? void 0 : _a.stopAutoRotate(); },
            reset: () => { var _a; return (_a = elRef.value) === null || _a === void 0 ? void 0 : _a.reset(); },
        });
        // ── Render ────────────────────────────────────────────────────────────────
        return () => {
            const boolAttr = (val) => (val ? '' : 'false');
            return h('viewer-360', Object.assign(Object.assign({ ref: elRef, frame: props.frame, speed: props.speed, 'rotation-direction': props.rotationDirection, 'dpr-cap': props.dprCap, 'min-scale': props.minScale, 'max-scale': props.maxScale, 'preload-count': props.preloadCount, 'concurrent-loads': props.concurrentLoads, 'show-controls': boolAttr(props.showControls), 'show-zoom-buttons': boolAttr(props.showZoomButtons), 'show-fullscreen-button': boolAttr(props.showFullscreenButton), 'show-loading-overlay': boolAttr(props.showLoadingOverlay), 'show-mobile-hint': boolAttr(props.showMobileHint) }, (props.autoRotate ? { 'auto-rotate': '' } : {})), (props.initialResolution != null ? { 'initial-resolution': props.initialResolution } : {})));
        };
    },
});
