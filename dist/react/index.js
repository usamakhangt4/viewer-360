 ...
 * ```
 */
const Viewer360 = forwardRef(function Viewer360(props, forwardedRef) {
    const { images, highResImages, frame = VIEWER_360_DEFAULTS.frame, autoRotate = VIEWER_360_DEFAULTS.autoRotate, speed = VIEWER_360_DEFAULTS.speed, rotationDirection = VIEWER_360_DEFAULTS.rotationDirection, initialResolution, dprCap = VIEWER_360_DEFAULTS.dprCap, zoomLevels = VIEWER_360_DEFAULTS.zoomLevels, minScale = VIEWER_360_DEFAULTS.minScale, maxScale = VIEWER_360_DEFAULTS.maxScale, showControls = VIEWER_360_DEFAULTS.showControls, showZoomButtons = VIEWER_360_DEFAULTS.showZoomButtons, showFullscreenButton = VIEWER_360_DEFAULTS.showFullscreenButton, showLoadingOverlay = VIEWER_360_DEFAULTS.showLoadingOverlay, showMobileHint = VIEWER_360_DEFAULTS.showMobileHint, preloadCount = VIEWER_360_DEFAULTS.preloadCount, concurrentLoads = VIEWER_360_DEFAULTS.concurrentLoads, onReady, onFrameChange, onZoomChange, onAutoRotateChange, onClose, onError, className, style, } = props;
    ...
    useEffect(() => {
        const el = innerRef.current;
        if (el && highResImages)
            el.highResImages = highResImages;
    }, [highResImages]);
    useEffect(() => {
        const el = innerRef.current;
        if (el)
            el.zoomLevels = zoomLevels;
    }, [zoomLevels]);
    // ── Event listeners ───────────────────────────────────────────────────────
    useEffect(() => {
        const el = innerRef.current;
        ...
        if (onReady)
            on('viewer360:ready', () => onReady());
        if (onFrameChange)
            on('viewer360:framechange', (e) => onFrameChange(e.detail));
        if (onZoomChange)
            on('viewer360:zoomchange', (e) => onZoomChange(e.detail));
        if (onAutoRotateChange)
            on('viewer360:autorotatechange', (e) => onAutoRotateChange(e.detail));
        if (onClose)
            on('viewer360:close', () => onClose());
        if (onError)
            on('viewer360:error', (e) => onError(e.detail));
        return () => {
    ...
    return (jsx("viewer-360", Object.assign({ ref: setRef, className: className, style: style, frame: frame, speed: speed, "rotation-direction": rotationDirection, "dpr-cap": dprCap, "min-scale": minScale, "max-scale": maxScale, "preload-count": preloadCount, "concurrent-loads": concurrentLoads }, (autoRotate ? { 'auto-rotate': '' } : {}), (initialResolution != null ? { 'initial-resolution': initialResolution } : {}), { "show-controls": boolAttr(showControls), "show-zoom-buttons": boolAttr(showZoomButtons), "show-fullscreen-button": boolAttr(showFullscreenButton), "show-loading-overlay": boolAttr(showLoadingOverlay), "show-mobile-hint": boolAttr(showMobileHint) })));
});
